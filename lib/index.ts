...
export const printers = async (
	...
	if (id != null) {
		...
		const result: string = await invoke(
			"plugin:printer|get_printers_by_name",
			{
				printername,
			}
		);
...
export const print = async (
		...
		if (typeof printerSettings.range == "string") {
			...
			rangeStr =
				printerSettings.range[printerSettings.range.length - 1] != ","
					...
					: printerSettings.range.substring(
							0,
							printerSettings.range.length - 1
					  );
	...
	const optionsParams: any = {
		...
		remove_after_print:
			typeof options.remove_temp != undefined
				? options.remove_temp
				: true,
	};
 ...
 * @returns Success.
 */
export const remove_files = async (
	paths: string[]
): Promise<ResponseResult> => {
	try {
